...
var isHouse = window.location.href.includes("house-attendance")
...
fetch(url, {
    ...
    .then(function (myData) {
        ...
        var loadings = document.getElementsByClassName("sk-cube-grid");
        for (var i = 0; i < loadings.length; i++) {
            loadings[i].style.display = 'none';
        }
        document.getElementById(where + "-glance").style.display = "table";
        document.getElementById(where + "-least").style.display = "table";
        document.getElementById(where + "-most").style.display = "table";
...
//Figure out the information that will go into the top table (number of political affeliates, percentage of voting with their own party)

function getTopTableInfo() {
    for (var i = 0; i < members.length; i++) { //Create a 'count' that will go through the members from the JSON
        if (members[i].party === "D") { //If the member of a party is a 'Democrat'
            statistics.parties[0].number_of_members += 1; //Then add that member to the object containing number of political affiliates
